...
def main():
    ...
    gene_columns = [col for col in all_columns if col != TARGET]
    use_columns = [TARGET] + gene_columns
    ...
    logger.info("Filtering out non-informative genes...")
    non_target = df.drop(columns=[TARGET])
    informative_genes = [col for col in non_target.columns if df[col].nunique() > 1]
